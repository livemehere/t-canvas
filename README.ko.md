...
* 전용 Crop Transformer로 이미지 가장자리를 잘라낼 수 있습니다. crop된 만큼 이미지 프레임도 함께 줄어듭니다.
...
* `Save`/`Load` 또는 `Cmd + S`/`Cmd + O`로 `.tcanvas` 문서를 저장하고 불러올 수 있습니다.
...
| Line | `L` | 드래그해서 선을 생성합니다. Shift로 수평/수직을 고정합니다. |
| Arrow | `A` | 드래그해서 화살표를 생성합니다. Shift로 수평/수직을 고정합니다. |
| Text | `T` | 텍스트 박스를 만들고 내용을 편집합니다. |
| Image | `I` | 이미지 파일을 가져오고 crop할 수 있습니다. |
...
* `[`/`]`로 선택한 레이어를 한 단계씩 뒤/앞으로 이동할 수 있습니다.
* `Shift + [`/`]`로 선택한 레이어를 최하단/최상단으로 이동할 수 있습니다.
...
이미지 도형에서 Inspector의 `Crop Image`를 누르면 Crop Mode가 시작됩니다. Crop 핸들을 드래그해 가장자리를
잘라내고 `Exit Crop Mode` 또는 `Esc`로 일반 Transformer로 돌아갈 수 있습니다.
...
툴바의 `Save` 또는 `Cmd + S`로 현재 캔버스를 `.tcanvas` 파일로 저장하고, `Load` 또는 `Cmd + O`로 불러올 수
있습니다. 도형 속성, 레이어 순서, 선택 상태, 캔버스 pan/zoom, crop 상태, 브러시 경로와 PNG 이미지 데이터를
파일 안에 함께 저장하므로 원본 파일이 없어도 붙여넣은 이미지를 복원할 수 있습니다.
...
앱 종료 시 macOS가 생성한 파일 패널 helper 프로세스도 함께 정리합니다.
